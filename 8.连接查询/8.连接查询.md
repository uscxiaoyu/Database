...
|A.a | A.b | A.c | B.c | B.d | B.f |
| --:| --: |---: | --:| --: |---: |
|1 |2 |3 | `NULL` |xxxxxxxxxx SELECT *FROM departmentWHERE budget > ALL(SELECT budget FROM department WHERE dept_name = 'Biology' OR dept_name = 'Physics');-- 或者SELECT *FROM departmentWHERE budget > (SELECT MAX(budget) FROM department WHERE dept_name = 'Biology' OR dept_name = 'Physics');​sql |`NULL` |
