...
值得注意的是，一些商业关系型数据库的表并不等于理论上的关系，例如MySQL的表中若未设置主键约束，则可以存在重复行或存在空行，而这对于关系数据库理论中的关系而言是不允许的。此外，在实际应用中，一般不会使用交叉连接。原因在于交叉连接将产生大量的中间数据，占用大量内存。一般情况下，建议将交叉连接与选择条件一起构成内连接或者自然连接，以减少中间过程产生的数据量。
