...
低版本的`MySQL`没有行号函数，可通过定义变量来模拟实现。
...
### 2. 给`prepare statement`提供值

```mysql
prepare stmt from "select product_id, product_place, price from product where price > ? order by price";
