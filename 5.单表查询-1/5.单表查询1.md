...
#### 示例17: 查找product表中商品名称末尾字符串为有复印机的商品名称，价格和产地
...
### 4.1 带`AND`关键字的多条件查询: 连接两个或者多个查询条件
