...
- （2）查询member表中user_name, address, email对应的数据，返回10~19行。
...
> 注意: 在SQL中，`a <= x <= b`并不等价于 `x >= a and x <= b`
> 请自行对比以下两个查询结果:
...
> 注：`count(*)`可用于统计返回结果的行数；
> `100 <= price <= 500`返回表中的记录数, 先运算`100 <= price`，再将运算结果(`true或false`)与`500`进行比较，显然对于每一行有price值且大于等于0的行都成立.

### 2. 带`in`关键字的查询：限定结果在某一集合中
...
#### 示例20：找出`product`中商品名称含'复印机'或'过胶机'的商品记录
...
#### 示例21: 找出商品名称含'复印机'和'过胶机'，且产地为天津的的商品记录
...
- 找出商品名称中含有“理光”和“墨粉”的商品记录。（理光公司的墨粉产品）

- 找出Product_Place不为空的计算器商品记录，显示10条记录。
