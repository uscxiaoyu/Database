...
> - `MERGE`：在涉及视图的SQL语句中，将视图定义取代查询语句的对应部分；在执行效率上比`temptable`更加高效。
> - `TEMPTABLE`: 在涉及视图的SQL语句中，将视图结果存入临时表，然后在临时表的基础上再执行语句；可以不在视图定义涉及的表上加锁，利于并发。
>
...
start transaction;
insert into view_view_product(product_id, price, sort_id)
values (9999, 10000, 12); -- 出错，cascade选项级联检查view_product的条件
rollback;
