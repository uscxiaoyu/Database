...
> - 定义外键名时，不能加单引号或者双引号。如：`constraint 'FK_ID'` 或 `constraint "FK_ID"`都是错误的；
...
索引是一种特殊的数据库结构，其作用相当于一本书的目录，以在查询时快速地定位到目标行。索引是提高数据查询效率的重要方式之一，是提高性能的最常用的工具之一。用户创建的索引指向数据库中具体数据所在位置。当用户通过索引查询数据库中的数据时，不需要遍历表中所有数据行。
